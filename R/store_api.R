...
StoreApi <- R6::R6Class(
  ...
  public = list(
    ...
    GetInventoryWithHttpInfo = function(...){
      ...
      if (httr::status_code(resp) >= 200 && httr::status_code(resp) <= 299) {
        deserializedRespObj <- tryCatch(
          self$apiClient$deserialize(resp, "map(integer)", loadNamespace("rpetstore")),
          error = function(e){
    ...
    GetOrderByIdWithHttpInfo = function(order.id, ...){
      ...
      if (httr::status_code(resp) >= 200 && httr::status_code(resp) <= 299) {
        deserializedRespObj <- tryCatch(
          self$apiClient$deserialize(resp, "Order", loadNamespace("rpetstore")),
          error = function(e){
             stop("Failed to deserialize response")
          }
        )
