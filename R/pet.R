  ...
  public = list(
    ...
    fromJSON = function(PetJson) {
      ...
      if (!is.null(PetObject$`photoUrls`)) {
        self$`photoUrls` <- ApiClient$new()$deserializeObj(PetObject$`photoUrls`, "array[character]", loadNamespace("rpetstore"))
      }
      if (!is.null(PetObject$`tags`)) {
        self$`tags` <- ApiClient$new()$deserializeObj(PetObject$`tags`, "array[Tag]", loadNamespace("rpetstore"))
      }
